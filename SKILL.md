...
Use Jina Reader to find the CMC coin ID from the image URL pattern on the page:

```bash
curl -s "https://r.jina.ai/{cmcUrl}" | grep -o 's2\.coinmarketcap\.com/static/img/coins/64x64/[0-9]*\.png' | head -1
```

The number in that URL is the CMC coin ID. Also parse the symbol if not provided by the user:

```bash
curl -s "https://r.jina.ai/{cmcUrl}" | grep -i "symbol\|ticker" | head -5
```

Download the PNG using Python (curl exits with code 56 on CMC CDN). CMC CDN max is only 200px — for better quality, also try CoinGecko `/original/` (often 2000px+):

```python
import urllib.request, os

cmc_id = '{ID}'      # extracted above
symbol  = '{SYMBOL}' # from user or parsed, UPPERCASE

url = f'https://s2.coinmarketcap.com/static/img/coins/200x200/{cmc_id}.png'
...
def make_svg(stroke_color=None):
    stroke_el = (
        f'<circle cx="96" cy="96" r="94" fill="none" stroke="{stroke_color}" stroke-width="4"/>'
...
def patch_svg(text, stroke_color=None):
    stroke_el = (
        f'\n  <circle cx="96" cy="96" r="94" fill="none" stroke="{stroke_color}" stroke-width="4"/>'
        if stroke_color else ''
    )
...
**CMC coin ID**: Found in the image URL on the CMC page — `s2.coinmarketcap.com/static/img/coins/64x64/{ID}.png`. Use this ID to fetch the 200x200 version.

**CMC RWA (stocks/real-world assets)**: URLs like `/real-world-assets/tesla/` use a different image path: `s2.coinmarketcap.com/static/img/rwa/64x64/{ID}.png` (not `/coins/`). Grep for both `/coins/` and `/rwa/` patterns. Match the logo labeled with the token name (e.g., `![Image N: TSLA]`) — sidebar images are unrelated coins.
